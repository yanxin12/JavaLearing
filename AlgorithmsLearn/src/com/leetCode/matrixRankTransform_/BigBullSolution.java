...
public class BigBullSolution {
    ...
    // 将多位数据转化为一维数据进行保存和处理，Index中的值可以和对应的序号进行转换
    public int[][] matrixRankTransform(int[][] matrix) {
        ...
        //初始化indexs和并查集p
        // 并查集初始化的时候，每一个数据都有一个父节点且为自己
        // index中的序号可以转化为对应矩阵的位置
        for (int i = 0; i < row * col; i++) {
            indexs[i] = i;
            p[i] = i;
        }

        //按照矩阵中的值排序indexs（大佬的高超之处）
        Arrays.sort(indexs, (Integer i, Integer j) -> (matrix[i / col][i % col] - matrix[j / col][j % col]));

        //由于经过排序后，小的元素先考虑，如果出现更新的位置(i, j) 所在的行、列已经更新过，
        // 那么当前位置的秩必然大于等于已经更新过的位置的秩，因此记录i行，j列之前最后一次更新秩的索引，
        // 然后根据索引找到最后一次更新的秩的值，从行列取到最大值就是(i, j)位置的秩了。
        ...
        while (pos < row * col) {
            ...
            //若i行中有更新过的位置
            if (rowMaxRank[i] != -1) {
                //获取最后一次更新过的下标，以及秩的值，因为矩阵中存在相同的数字
                int k = rowMaxRank[i];
                int tmpIdx = i * col + k;
            ...
            //因为排完序之后是从小的值往大的值进行遍历，所以每次遍历的数值都是最大的值，更新最大秩的索引
            rowMaxRank[i] = j;
            colMaxRank[j] = i;
